...
The Country have 243 unique values but it also include continent including Africa, Antarctica, Asia, Europe, North America, Oceania, South America.

```{r}
unique(temperature$Country)
```
Interestingly, there are some country which represented in 2 different values: Denmark (Europe) - Denmark, France (Europe) - France, Netherlands (Europe), Netherlands, and United Kingdom (Europe) - United Kingdom.

```{r}
temperature %>%
  filter(Country %in% c('Denmark (Europe)', 'Denmark')) %>%
  ggplot(aes(x=dt, y=AverageTemperature, color=Country)) +
    geom_line() +
    labs(title = 'Denmark temperature')

temperature %>%
  filter(Country %in% c('France (Europe)', 'France')) %>%
  ggplot(aes(x=dt, y=AverageTemperature, color=Country)) +
    geom_line() +
    labs(title = 'France temperature')

temperature %>%
  filter(Country %in% c('Netherlands (Europe)', 'Netherlands')) %>%
  ggplot(aes(x=dt, y=AverageTemperature, color=Country)) +
    geom_line() +
    labs(title = 'Netherlands temperature')

temperature %>%
  filter(Country %in% c('United Kingdom (Europe)', 'United Kingdom')) %>%
  ggplot(aes(x=dt, y=AverageTemperature, color=Country)) +
    geom_line() +
    labs(title = 'United Kingdom temperature')
```
...
The Entity have 247 unique values including countries but also
...
Remove rows with continent or redundant Europe record in Country column. Now we have 232 uniqe values.
...
Remove rows with invalid country in Entity column. Now we have 221 unique values.
...
unique(selectedCo2Emission$Entity)
```
...
# Research Questions 2: The evil emitters and unlucky receivers
...
Since the annual temperatures were swinging between each each. So I create an estimation of temperature change of each country between 1913 and 2012 using the linear regression model with the land temperature dataset.
...
The distribution is very close to the normal distribution which is unexpected from us.
...
Then we find total CO2 emission from 1913 - 2012 of each country from CO2 emission dataset.
...
```{r}
ggplot(countryTemperatureChangeAndEmission, aes(x = Group)) +
  geom_bar() + 
  labs(title = 'Number of country in each group')
```

Here is the country of each group with temperature changes and CO2 emission of each country from 1913-2012
...
```
The above chart is difficult to read. Since a lot of countries has relatively low emission, and we are not interested in neutral country, here is the plot that filter out neutral country:
